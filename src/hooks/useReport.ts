...
// Lazy-loaded to match useNotifications / usePushNotifications / useSafeCheckIn.
...
export function useReport() {
  ...
  const submitReport = async (payload: ReportPayload): Promise<SubmitResult> => {
    ...
    try {
      // Use getSession() for cache-read reliability (avoids getUser() storage auth pitfalls)
      ...
      // 2. Insert report row with quality token and media_paths included
      const insertPayload = {
        user_id: user.id,
        category: payload.category,
